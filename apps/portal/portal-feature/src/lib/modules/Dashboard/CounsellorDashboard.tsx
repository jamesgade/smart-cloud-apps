...
const CounsellorDashboard: React.FC = () => {
  ...
  React.useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        // Assigned count restricted to this counsellor
      ...
      try {
        // Assigned students for current counsellor
      ...
      try {
        // Applications summary for this counsellor from backend
      ...
      try {
        // Followups for assigned students (backend returns assigned students' followups for counsellor)
    ...
    <Box sx={{ p: 3 }}>
      <Box sx={{ mb: 2 }}>
        <Typography variant="h4" component="h1" sx={{ fontWeight: 700 }}>
          {`Welcome, ${identity?.userName || identity?.firstName || 'Counsellor'}`}
        </Typography>
      </Box>
        ...
        <CardContent>
          <Box sx={{ display: 'flex', flexDirection: { xs: 'column', sm: 'row' }, alignItems: { xs: 'stretch', sm: 'center' }, justifyContent: 'space-between', gap: 2 }}>
            <Box>
              ...
              <Typography variant="body2" color="text.secondary">Schedule sessions, message students, or add followups.</Typography>
            </Box>
