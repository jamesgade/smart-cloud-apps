...
const CounsellorMessaging: React.FC = () => {
  ...
  const loadConversations = async () => {
    try {
      setLoading(true);
      setError(null);
      const response = await axiosInstance.get(`${API_BASE_URL}/messaging/counsellor/conversations`);
  ...
  const loadMessages = async (conversationId: string) => {
    try {
      const response = await axiosInstance.get(`${API_BASE_URL}/messaging/counsellor/conversations/${conversationId}/messages`);
  ...
  const sendMessage = async () => {
    ...
    try {
      setSending(true);
      const response = await axiosInstance.post(`${API_BASE_URL}/messaging/counsellor/messages`, {
              ...
              <>
                  ...
                  {messages.length > 0 ? (
                    messages.map((message) => (
                      <Box
                        ...
                        sx={{
                          display: 'flex',
                          justifyContent: message.senderType === 'COUNSELLOR' ? 'flex-end' : 'flex-start',
                          mb: 2
                        }}
                      >
                        <Paper
                          sx={{
                            p: 2,
                            maxWidth: '70%',
                            backgroundColor: message.senderType === 'COUNSELLOR' ? '#FF6B35' : '#f5f5f5',
                            color: message.senderType === 'COUNSELLOR' ? 'white' : 'inherit',
                            border: !message.isRead && message.senderType !== 'COUNSELLOR' ? '2px solid #FF6B35' : 'none'
                          }}
                        >
                          <Typography 
                            variant="body2" 
                            sx={{ 
                              mb: 0.5,
                              fontWeight: !message.isRead && message.senderType !== 'COUNSELLOR' ? 700 : 400
                            }}
                          >
                            {message.messageText}
                          </Typography>
                          <Typography variant="caption" sx={{ opacity: 0.7 }}>
                            ...
                            {message.senderType !== 'COUNSELLOR' && message.senderName && ` • ${message.senderName}`}
                            {!message.isRead && message.senderType !== 'COUNSELLOR' && (
                ...
                <Box sx={{ p: 2, borderTop: '1px solid #eee' }}>
                  <Box sx={{ display: 'flex', gap: 1 }}>
                    <TextField
                      fullWidth
                      placeholder="Type your message as counsellor..."
                      value={newMessage}
